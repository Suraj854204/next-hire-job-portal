...
import { forgotPasswordTemplate } from "../templates/index.js";
...
export const loginUser = TryCatch(async (req, res, next) => {
  ...
  SELECT u.user_id, u.name, u.email, u.password, u.phone_number, u.role, u.bio, u.resume, u.profile_picture, u.subscription, ARRAY_AGG(s.name) FILTER (WHERE s.name IS NOT NULL) as skills FROM users u LEFT JOIN user_skills us ON u.user_id = us.user_id
...
export const submitComplaint = TryCatch(async (req, res, next) => {
  ...
  publishToTopic("send-mail", messageData).catch((err) => {
